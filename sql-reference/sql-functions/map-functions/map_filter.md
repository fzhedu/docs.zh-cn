...
根据设定的过滤函数返回 MAP 中匹配的 key-value 对。该过滤函数可以是普通的 Boolean 数组，也可以是灵活的 Lambda 函数。有关 Lambda 函数的详细信息，参见 [Lambda expression](../Lambda_expression.md)。该函数从 3.0 版本开始支持。
...
返回 map, 类型跟输入的 `any_map` 相同。

如果输入参数 `any_map` 是 NULL，结果也是 NULL; 如果 `array<boolean>` 是 null，结果是空 MAP。
